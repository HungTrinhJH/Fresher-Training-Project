...
import css from './ManufactureYearFilter.module.css';
import { numberAtLeast, composeValidators } from '../../util/validators';
...
const getManufactureYearQueryParam = queryParamNames => {
...
class ManufactureYearFilter extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    const urlParam = getManufactureYearQueryParam(queryParamNames);
    ...
    const labelForPlain = hasInitialValues
      ? intl.formatMessage(
          { id: 'ManufactureYearFilter.labelSelected' },
          { labelText: initialValues[urlParam] }
        )
      : label;
    ...
    const placeholder = intl.formatMessage({ id: 'ManufactureYearFilter.placeholder' });

    const contentStyle = this.positionStyleForContent();

    const validateAtLeastYearMsg = intl.formatMessage(
      {
        id: 'ManufactureYearFilter.atLeastYearMsg',
      },
      {
        year: rest.min,
      }
    ...
    const manufactureYearAtLeast = numberAtLeast(validateAtLeastYearMsg, rest.min);
    const manufactureYearValidator = composeValidators(manufactureYearAtLeast);
    ...
    return showAsPopup ? (
      ...
      >
        <FieldTextInput
          ...
          type="number"
          label={filterText}
          validate={manufactureYearValidator}
...
ManufactureYearFilter.defaultProps = {
  rootClassName: null,
  className: null,
  initialValues: null,
...
ManufactureYearFilter.propTypes = {
...
export default injectIntl(ManufactureYearFilter);
